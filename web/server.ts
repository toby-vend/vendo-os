...
import { adminFrameioMappingRoutes } from './routes/admin/frameio-mapping.js';
...
import { squatFunnelRoutes } from './routes/squat-funnel.js';
...
import { frameIoLibraryRoutes } from './routes/dashboards/frame-io-library.js';
...
app.addHook('onRequest', async (request, reply) => {
  ...
  if (path.startsWith('/assets/') || path === '/login' || path === '/auth/google/login' || path === '/auth/google/callback' || (path.startsWith('/onboard') && !path.startsWith('/onboarding')) || path === '/api/drive/webhook' || path === '/api/fathom/webhook' || path === '/api/frameio/webhook' || path === '/api/boxly/webhook' || path === '/api/slack/interact' || path.startsWith('/api/squat/')) return;
...
app.register(clientKnowledgeRoutes, { prefix: '/clients' });
...
app.register(creativePipelineRoutes, { prefix: '/creative-pipeline' });
...
app.register(adminFrameioMappingRoutes, { prefix: '/admin/frameio-mapping' });
...
app.register(adminAgentsDetailRoutes, { prefix: '/admin/agents' });
...
app.register(squatFunnelRoutes, { prefix: '/api/squat' });
...
app.register(frameIoLibraryRoutes, { prefix: '/dashboards/frame-io/library' });
