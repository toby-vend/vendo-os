    ...
    try {
      ...
      try {
        const aiInput = {
          ...
          // AGENT-COORD: googleAdsSummary becomes a typed field on
          // ReportAiInput once A2 lands the report-ai.ts extension.
        } as Parameters<typeof generateReportInsights>[0];
        const out = await generateReportInsights(aiInput, null);
