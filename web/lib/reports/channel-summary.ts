...
function renderMeta(block: MetaBlock): { text: string; hasData: boolean } {
  ...
  lines.push('');
  lines.push('Overall:');
  if (spend) lines.push(`- Spend: ${gbp(spend.value)}`);
  if (leads) lines.push(`- Leads: ${num(leads.value)}`);
  if (cpl) lines.push(`- CPL: ${gbp(cpl.value)}`);
  ...
  for (const c of active) {
    ...
    lines.push(`- CPL: ${gbp(c.cpl)}`);
    if (c.revenue > 0) lines.push(`- Revenue: ${gbp(c.revenue)}`);
  }
...
export async function buildChannelSummary(
  ...
  if (channel === 'meta') {
    ...
    const { text, hasData } = renderMeta(block);
