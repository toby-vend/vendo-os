 ...
 * Paid Search reporting. PRESENTATION here (prose, "leads"/"cost per lead",
...
- exec_summary = "Headline": 2 to 3 sentences. The topline (total leads,
...
- performance_summary = "Performance by campaign": a markdown TABLE
  (Campaign | Leads | Cost per lead | Spend, + Total row; no % of budget
  column), then ONE short paragraph reading the allocation honestly.
- worked_on = "What we did this month": deduped bullets, active voice, each
  opening with a short bold label (same format as "Next month").
- risks = "Watch point": the single key issue, owned, framed as a fix-it item
  with a reassess date.
- recommendations = "Next month": specific actions with goals, each opening
  with a short bold label. Add a closing "One thing I need from you:" ask ONLY
  when the team's input explicitly contains one; otherwise end on the actions.
...
- Show the spend split so allocation is visible (Spend column, no % of budget
  column). Own the trade-off (e.g. "a third of budget for ~5% of leads") and
  don't excuse it.
- Earn adjectives: only "strong/efficient/etc." with a benchmark (MoM or
  target). Otherwise drop the word and let the number stand.
- Anchor to bookings/revenue when that data exists, as booking is the real goal.
- Do NOT manufacture a closing ask. End on the next-month actions. Add a
  decision request ONLY when the team's input explicitly contains one.
- NO em dashes (—) anywhere. No en dashes (–) as punctuation. Use commas, full
  stops, colons, or brackets instead.

TERMINOLOGY:
- Lead-gen (dental, services, B2B): always call enquiries "leads". Use "leads",
  "cost per lead" (£X per lead), "form submissions", "phone calls", "bookings".
  Not "conversions"/"CPR".
...
export const FEW_SHOT_EXAMPLES: FewShotExample[] = [
  {
    ...
    // order. Note: each number stated once, spend split visible (no % of budget
    // column), trade-off owned, bold-led bullets in both lists, and it ends on
    // the focus with no manufactured ask. Match this exactly.
...
[HEADLINE]
112 leads at a blended cost per lead of £23.61, from £2,644.32 spend. General Dentistry did the heavy lifting; Performance Max was the most efficient channel; Clear Aligners remains low-volume and is the main thing to address.
...
General Dentistry is delivering volume efficiently and remains the engine for filling diaries. Performance Max is punching above its weight and is worth scaling. Clear Aligners is currently taking about a third of the budget for around 5% of leads, so the priority this month is to make that spend work harder rather than let it ride.

[WHAT WE DID THIS MONTH]
- **Account optimisations:** completed quarterly, monthly and bi-weekly optimisations, plus a full account audit.
- **Landing page fix:** corrected a form issue on the Clear Aligners landing page to keep the lead journey friction-free.
- **Thank-you page:** updated the website thank-you page.
- **CRM migration:** progressed the Boxly CRM migration, coordinating the lead-flow switchover to avoid disruption to incoming leads.
- **Geographic targeting:** reviewed targeting and prepared zone recommendations to apply going forward.

[WATCH POINT: CLEAR ALIGNERS]
6 leads at £155.48 is well above the rest of the account. A higher cost per lead is expected for this treatment, but at current volume the spend is hard to justify on its own. Rather than simply accept it, we're treating this as a fix-it item (see next month) and will reassess at the end of June.
...
- **CRM lead-flow:** ensure all Google Ads leads feed cleanly into Boxly with automated follow-up, to speed up response and improve lead-to-booking.
