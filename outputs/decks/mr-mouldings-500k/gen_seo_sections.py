...
<div class="card" style="display:flex;gap:40px;align-items:center;padding:28px 30px"><div style="flex:0 0 150px"><p class="lbl">Where organic sits in the £500k</p><p class="stat" style="font-size:52px;color:{AMBER}">7.6%</p></div>
...
cats=['1–3','4–10','11–20','21+']; imp=[14.3,44.8,34.4,6.6]; clk=[26.0,53.0,18.6,2.4]
bars=''
for c,i,k in zip(cats,imp,clk):
    hi=round(i/60*340); hk=round(k/60*340)
    bars+=f'''<div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-end;height:400px">
<div style="display:flex;gap:6px;align-items:flex-end">
<div style="display:flex;flex-direction:column;align-items:center;gap:4px"><span style="font-size:13px;color:#B9C7C0">{i}</span><div style="width:52px;height:{hi}px;background:#2F5A50;border-radius:3px 3px 0 0"></div></div>
<div style="display:flex;flex-direction:column;align-items:center;gap:4px"><span style="font-size:13px;color:{MINT};font-weight:700">{k:g}</span><div style="width:52px;height:{hk}px;background:{MINT};border-radius:3px 3px 0 0"></div></div>
</div><p style="margin:10px 0 0;font-size:14px;color:#B9C7C0">Positions {e(c)}</p></div>'''
...
<div style="display:flex;gap:20px;margin:6px 0 10px"><span style="font-size:13px;color:#B9C7C0"><span style="display:inline-block;width:11px;height:11px;background:#2F5A50;margin-right:6px"></span>Share of impressions %</span><span style="font-size:13px;color:#B9C7C0"><span style="display:inline-block;width:11px;height:11px;background:{MINT};margin-right:6px"></span>Share of clicks %</span></div>
<div style="display:flex;border-bottom:1px solid rgba(255,255,255,.12)">{bars}</div></div></div>
...
H['Seo2']=('SEO &#183; The ranking gap','Page one is where the <span class="fl">clicks</span> are','Share of impressions and clicks by Google position. UK, last 90 days, brand searches excluded.')
...
('Dec','/collections/torus-skirting-boards<br>/products/torus-architrave','torus skirting board 4,400 · torus architrave 1,900','11–13','Skirting: Google already picks the collection, so copy and links. Architrave: Google picks the Torus product page, so we strengthen that page.'),
('Dec','/collections/ovolo-architrave<br>/collections/stepped-architrave','ovolo architrave 720 · stepped architrave 210','14–17','Ovolo: the collection leads, so copy and links. Stepped: the collection and four stepped products split the term, so the products link to the collection.')]
tr=''.join(f'<tr><td style="color:{MINT};font-weight:700">{w}</td><td style="color:#fff;font-weight:600">{p}</td><td class="mut">{e(s)}</td><td class="n" style="color:{AMBER};font-weight:700">{e(pos)}</td><td class="mut" style="padding-left:18px">{e(d)}</td></tr>' for w,p,s,pos,d in rows)
...
lines=[('Solid Hardwood · 23,400 searches',702,2200),('LED coving · 8,270 searches',248,777),('Ceiling roses · 8,000 searches',240,752),('Bobbin · 3,040 searches',91,286)]
hb=''.join(f'''<div style="margin-bottom:18px"><p style="margin:0 0 6px;font-size:14px;color:#fff">{e(n)}</p>
<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px"><div style="height:16px;width:{round(t/2200*520)}px;background:{MINT};border-radius:2px"></div><span style="font-size:13px;color:{MINT};font-weight:700">{t:,}</span></div>
<div style="display:flex;align-items:center;gap:8px"><div style="height:16px;width:{round(f/2200*520)}px;background:#2F5A50;border-radius:2px"></div><span style="font-size:13px;color:#B9C7C0">{f:,}</span></div></div>''' for n,t,f in lines)
S['Seo6']=f'''<div class="bd">
<div class="col" style="flex:1.5"><div class="card" style="height:100%"><p class="lbl">Visits a month &#183; UK &#183; search total beside each line</p>
<div style="display:flex;gap:20px;margin:6px 0 16px"><span style="font-size:13px;color:#B9C7C0"><span style="display:inline-block;width:11px;height:11px;background:{MINT};margin-right:6px"></span>At third place</span><span style="font-size:13px;color:#B9C7C0"><span style="display:inline-block;width:11px;height:11px;background:#2F5A50;margin-right:6px"></span>At first place</span></div>{hb}</div></div>
<div class="col" style="flex:1">
...
SECTIONS=[
 ...
 ('Div7','The plan',['Twelve months','Budget and cash','What we need','The first thirty days']),
 ('Div8','Paid social',['Meta today','The plan','The forecast']),
 ('Div9','Website',['Five waves, ready to go live','Everything built','What the release is worth','The funnel, stage by stage','Recoverable revenue at checkout']),
]
...
H['Agenda']=('Agenda','Nine sections, one <span class="fl">plan</span>','Each channel takes its own time.')
